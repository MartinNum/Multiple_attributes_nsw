...
cdef extern from "n2/hnsw.h" namespace "n2":
    cdef cppclass Hnsw:
        ...
        void SearchByVector_new(const vector[float]& qvec,vector[string] attributes, size_t, size_t, vector[pair[int, float]]& result) nogil except +
...
cdef class _HnswIndex:
    ...
    def search_by_vector(self, _v, _s, _k, _ef_search):
        cdef vector[float] v = _v
        cdef vector[string] s = _s
        cdef size_t k = _k
        cdef size_t ef_search = _ef_search
        cdef vector[pair[int, float]] ret
        with nogil:
            self.obj.SearchByVector_new(v, s, k, ef_search, ret)
        return ret
...
class HnswIndex(object):
    ...
    def search_by_vector(self, v, s, k, ef_search=-1, include_distances=False):
        """
        Returns k nearest items by vector.

        :param v: A query vector
        :param s: A query vector
        :param k: k value
        ...
        if ef_search == -1:
            ef_search = k * 10
        ret = self.model.search_by_vector(v, s, k, ef_search)
