...
public:
    ...
    void SearchByVector_new(const std::vector<float>& qvec,std::vector<std::string> attributes, size_t k, size_t ef_search,
                        std::vector<std::pair<int, float> >& result);

    void SearchByVector_new_violence(const std::vector<float>& qvec,std::vector<std::string> attributes, size_t k, size_t ef_search,
                        std::vector<std::pair<int, float> >& result);
...
private:
    ...
    // 所有的节点属性的集合
    std::map<int,std::vector<std::string>> attributes_;
    // 所有属性id的数量
    int all_id_number_;
    //存放所有id-属性
